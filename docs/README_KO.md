...
[English](../README.md) | [中文文档](README_CN.md) | [日本語](README_JA.md) | **한국어** | [Deutsch](README_DE.md) | [Français](README_FR.md) | [Español](README_ES.md) | [Português](README_PT.md) | [Italiano](README_IT.md)
...
- [⚡ v1.21.0 업데이트 하이라이트](#-v1210-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
v1.21.0은 세 가지 주요 개선을 제공하는 **마이너 릴리스**입니다: 환상 엔티티를 방지하는 프리인게스트 요구사항 게이트, 작업 이력 패널, 스키마 일관성 Phase 1. 제9언어로 이탈리아어 추가, 불완전 페이지 클리너, 여러 버그 수정도 포함됩니다.

- **🛡️ 프리인게스트 요구사항 게이트 (Issue #164, PR #174).** 모든 소스 파일이 LLM 호출 **전**에 검증됩니다 — 빈 파일/공백만/frontmatter만 있는 노트는 거부되어 로컬 모델(Ollama 등)의 엔티티명 환각을 방지합니다. 콘텐츠 해시 중복 제거로 경로를 가로지르는 동일 파일을 감지합니다. 인터랙티브 인게스트에서는 중복 파일 재인게스트 전에 확인을 요청합니다. @Indexed-Apogrypha 기여.
- **📊 작업 이력 패널 (Issue #122, PR #171).** 검색·필터링 가능한 UI에서 최근 인게스트, 린트 보고서, 유지보수 실행을 표시. 인사이트 기반 시각화로 KPI 카드, 항목 상세, 클릭 가능한 페이지 링크 제공. 명령 팔레트: "View Ingestion History".
- **🔗 스키마 일관성 Phase 1 (Issue #124, PR #167).** 스키마 설정(`schema/config.md`)이 시스템 프롬프트와 생성 프롬프트의 단일 진실 소스가 되었습니다. 사용자 정의 섹션과 태그 어휘가 모든 LLM 호출에 주입됩니다.
- **🇮🇹 이탈리아어 (Issue #159, PR #159).** 플러그인 UI와 Wiki 출력이 제9언어로 이탈리아어를 지원. @FrancoTampieri 기여.
- **🧹 불완전 페이지 클리너 (Issue #170, PR #177).** 중단된 인게스트로 남은 부분 상태 Wiki 페이지가 시작 시 자동 정리. 페이지는 Obsidian의 `.trash`로 아카이브(복원 가능).
- **🔧 버그 수정.** 비중국어 UI에서의 하드코딩 중국어 오류 문자열(#172); 중복 항목으로 인한 인게스트 보고서 수 부풀림(#173).

로컬 LLM(Ollama, LM Studio) 사용자는 특히 이 버전으로의 업그레이드를 강력히 권장합니다 — 프리인게스트 게이트가 "빈 파일 환각" 종류 버그를 방지합니다(작은 모델이 빈 프롬프트에 대해 JSON 스키마를 채우기 위해 가짜 엔티티 이름을 만들어내는 문제).
...
- **🌍 전면 UI 국제화** — 플러그인 UI 9개 언어 지원(EN/ZH/JA/KO/DE/FR/ES/PT/IT), 269+ UI 필드 완전 번역, 자연스러운 로컬 표현
