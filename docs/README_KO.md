...
- [⚡ v1.20.3 업데이트 하이라이트](#-v1203-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
v1.20.3은 wiki 쓰기 경로의 3가지 잠재 버그를 수정하는 **패치 핫픽스**입니다. 새로운 기능 없음 — 3가지 수정 모두 처음부터 존재했어야 하는 동작을 복원하는 "정합성/잠재 버그" 수정입니다.

- **🔧 소스 페이지 slug 충돌 수정 (Issue #155, PR #156).** 두 소스 파일이 폴더를 가로질러 파일 이름을 공유할 때(예: Academy 강의의 `About this course.md` 11개), `slugify(basename)`이 둘 다에 동일한 slug를 생성하여 두 번째 수집 시 **조용히 덮어쓰기**가 발생했고, 모든 `[[sources/<slug>]]` 역링크가 잘못된 소스로 해결되었습니다. 수정: 모든 소스 slug는 이제 `<basename>_<6자리 전체 경로 FNV-1a 해시>` 형식입니다. 기존 vault 재수집 시 `sources/` 페이지가 이름이 변경되고 역링크는 즉시 업데이트됩니다. @Indexed-Apogrypha 기여.
- **🔧 `mergeFrontmatter` alias 중복 제거 (PR #154).** 반복 수집 시 `aliases` 배열이 무제한으로 증가할 수 있었습니다 — 실제 환경의 한 페이지에서 동일한 alias 블록이 ~15회(86줄 중복) 누적되었습니다. `mergeFrontmatter`는 이제 `enforceFrontmatterConstraints`와 정합하여 `fm.aliases`를 중복 제거합니다. @DocTpoint 기여.
- **🔧 Stage-4 `reviewed: true` 가드 (PR #158).** 관련 없는 노트를 수집할 때 LLM이 `reviewed: true` 페이지의 본문을 덮어쓸 수 있었습니다 — reviewed 잠금은 `createOrUpdatePage` 경로에서만 작동했고 Stage-4 경로에서는 작동하지 않았습니다. 수정: `updateRelatedPage`는 이제 `reviewed: true` 페이지를 `appendToReviewedPage`로 라우팅합니다. @DocTpoint 기여.
- **🛠 tsconfig 정리.** `lib`을 ES2021로 업데이트; 불필요한 `baseUrl` 제거.

폴더를 가로질러 동일 이름 파일을 여러 개 수집하는 사용자, `reviewed: true` 페이지 잠금을 사용하는 사용자는 특히 이 버전으로의 업그레이드를 강력히 권장합니다.
