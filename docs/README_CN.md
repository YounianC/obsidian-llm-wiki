...
**v1.11.0 新增**：插件现在要求通过连接测试后才能使用核心功能。升级用户会自动信任已有配置（`llmReady = true`），无需额外操作。如果切换 Provider 或修改 API Key，需要重新通过 Test Connection。

如果你从 **v1.11.0 之前**版本升级，建议运行一次 **维护 Wiki (Lint Wiki)** 以自动修复以下历史遗留问题：
- **双重嵌套链接 `[[[[...]]]]`**：log.md 中可能存在格式异常的链接，Lint 会自动检测并修复
- **对侧目录重复 stub**：entities/ 和 concepts/ 中可能存在的同名重复页面会被 slug 匹配识别

如果你从 **v1.7.11 之前**（或更早版本）升级，现有 Wiki 页面缺少多个版本迭代中引入的功能。升级后按以下步骤操作，将 Wiki 更新到最佳状态：

**1️⃣ 重建索引**
`Cmd+P` → **"重新生成索引"** — 重新构建 `wiki/index.md`，为每个页面条目附加别名信息，启用别名感知搜索（如搜索"DSA"能找到"DeepSeek-Sparse-Attention"）。旧版索引格式只列出页面标题。

**2️⃣ 运行维护 Wiki**
`Cmd+P` → **"维护 Wiki"** — 扫描整个 Wiki，显示：
- **🏷️ 缺失别名**：没有别名的页面（v1.7.11 之前生成的所有页面）。点击 **"Complete Aliases"** 让 LLM 批量生成翻译、缩写、变体名。这对后续重复检测至关重要。
...
**4️⃣ 启用并行页面生成**
设置 → **摄入加速**：
- **⚡ 页面生成并发度**：大多 Provider 建议设置为 3（v1.7.3 之前默认为 1/串行）。含 10+ 实体的源文件可加速 2–3 倍。
- **⏱️ 批次延迟**：从 300ms 开始。如遇限流请增大至 500–800ms。

**5️⃣ 检查新增设置项（v1.4.0–v1.7.x 引入）：**
- **🌐 Wiki 输出语言**（v1.6.5）：独立于界面语言——Wiki 可以用中文撰写而插件界面保持英文，反之亦然。
- **📊 提取粒度**（v1.6.2，v1.10.0 扩展）：五种选项控制 LLM 从源文件提取实体的深度：
...
- **🔄 自动维护**（v1.4.0）：可选的文件监听、定时 Lint、启动健康检查。全部默认关闭——仅在需要后台自动处理时启用。

> **🛡️ 安全说明**：并行生成使用 `Promise.allSettled` —— 某页失败不影响其他页面继续。失败页面会自动重试并指数退避。智能批量跳过（v1.7.7）自动检测已摄入文件，节省时间和 API 成本。
...
本次为**生产级性能更新**。摄入提取流程进行了根本性重构——页面列表不再嵌入每次 LLM 调用中，提取速度不再随 Wiki 规模增长而下降。

**核心改进：**

- **摄入提速约 80%。** 一篇短文章从原来 30–90 秒降至 5–15 秒。Wiki 越大，提升越显著。
- **提取质量明显提升。** 不再有 200K 字符的页面列表干扰 LLM，提取结果更聚焦源文件内容，不再出现从其他页面实体"幻觉"出无关概念的问题。
- **Wiki 规模不再拖慢单篇摄入。** 10000 页 Wiki 处理单篇文件的速度与 500 页 Wiki 完全一致。插件已具备大规模生产环境使用能力。
- **智能批次控制。** 短文 1–2 轮即完成，不再被强制多轮迭代。进度显示增加了批次计数和累计条目数。
- **确定性关联页面匹配。** 新提取内容与已有页面的交叉引用改用程序化 slug+alias 匹配，比 LLM 推测更可靠、零额外成本。

**从旧版本升级？** 升级后运行一次 **Lint Wiki** 即可自动修复历史遗留问题。已有配置自动保留，无需重新设置。
...
**安装后为什么不能使用摄入/维护/查询功能？(v1.11.0)**
插件现在要求通过连接测试后才能使用核心功能。前往 **设置 → Karpathy LLM Wiki** → 选择提供商 → 填入 API Key → 点击 **Fetch Models** → 选择模型 → 点击 **Test Connection**。看到绿色 "LLM 已就绪" 指示后即可正常使用。这是为了防止配置错误导致的静默失败。

**如何取消正在运行的摄入或 Lint？(v1.11.0)**
摄入运行时点击状态栏的 "提取中... 点击取消" 文字，或使用 `Ctrl+P` → "取消当前提取"。操作会在当前批次完成后安全停止，保留所有已完成的工作。

**如何快速摄入当前正在编辑的文件？(v1.11.0)**
点击左侧边栏的 `sticker` 图标，或使用 `Ctrl+P` → "摄入当前文件"，无需文件选择器，直接摄入当前活动编辑器中的文件。

**log.md 中出现 `[[[[entities/Foo|Foo]]]]` 双重括号怎么办？**
这是 v1.11.0 之前的一个 bug (#37)。运行一次 **维护 Wiki (Lint Wiki)** 即可——扫描器会自动检测并修复整个 wiki 目录（包括 log.md）中的所有双重嵌套 wiki 链接，零 LLM 成本，无需手动清理。

**为什么会出现 "Overloaded" 错误且不重试？**
v1.11.0 之前的版本无法识别 Anthropic 的 529 过载错误为可重试。v1.11.0 已修复——过载错误现在会自动以指数退避重试，适用于所有提供商。

**为什么 entities/ 和 concepts/ 中会出现重复的 stub 页面？**
v1.11.0 之前，stub 创建只做精确标题匹配。如果 "机器学习" 作为概念已存在，但断链指向 `entities/机器学习`，就会创建重复 stub。v1.11.0 增加了 slug 匹配——同名的不同格式（如空格 vs 连字符）现在能正确匹配到已有页面。

**为什么提取现在会跳过引用作者和研究名称？**
v1.11.0 将提取标准从"文本中出现了什么？"改为"什么值得成为 wiki 页面？"——学术引用（作者名、试验名称）现在被识别为证据容器，而非 wiki 实体。它们的发现会被提取为概念，而非引用本身。
