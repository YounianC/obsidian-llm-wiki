...
v1.20.3 是一个**补丁版本热修复**，修复了 wiki 写入路径中的三个潜在 bug。无新功能——三个修复都属于"对齐性/潜在 bug"修复，恢复从一开始就应该存在的行为。

- **🔧 源页面 slug 冲突修复（Issue #155, PR #156）。** 当两个源文件跨文件夹共享文件名（如 11 份 Academy 课程的 `About this course.md`）时，`slugify(basename)` 为两者生成相同 slug——第二次摄入会**静默覆盖**第一次，所有 `[[sources/<slug>]]` 反向链接都指向错误的源。修复：每个源 slug 现在是 `<basename>_<6位全路径 FNV-1a 哈希>`。对已有 vault 重新摄入会重命名 `sources/` 页面，反向链接原地更新。由 @Indexed-Apogrypha 贡献。
- **🔧 `mergeFrontmatter` alias 去重（PR #154）。** 重复摄入会无界增长 `aliases` 数组——某真实 vault 的一个页面累积了 ~15 次相同 alias 块（86 行重复）。`mergeFrontmatter` 现在与 `enforceFrontmatterConstraints` 对齐去重 `fm.aliases`。由 @DocTpoint 贡献。
- **🔧 Stage-4 `reviewed: true` 守卫（PR #158）。** 摄入不相关的笔记可能用 LLM 重写已 `reviewed: true` 的页面 body——reviewed 锁定只在 `createOrUpdatePage` 路径生效，Stage-4 路径未生效。修复：`updateRelatedPage` 现在将 `reviewed: true` 页面路由到 `appendToReviewedPage`。由 @DocTpoint 贡献。
- **🛠 tsconfig 整理。** `lib` 升至 ES2021；移除无用的 `baseUrl`。

我们强烈建议所有用户升级到本版本，特别是：跨文件夹摄入多个同名文件的用户，以及使用 `reviewed: true` 页面锁的用户。
