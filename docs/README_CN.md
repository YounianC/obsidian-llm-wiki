...
v1.20.0 是一个**次版本更新**，重新设计了插件对 LLM 思考/推理的处理方式。插件默认不再发送 Provider 特定的思考控制字段——由 Provider 自行决定行为。当出现思考内容时（如 DeepSeek reasoning），会以可折叠面板显示在答案上方，类似 ChatGPT/Claude.ai 风格。同时包含多个 Provider 兼容性修复和 UX 改进。

- **🧠 Provider 优先的思考控制。** 默认模式不发送任何思考控制字段——由 Provider 自行决定是否推理。用户可在自定义高级设置中显式启用"禁用思考"，触发 3 级方言回退（thinking.type → reasoning_effort → 无）。
- **💭 可折叠思考 UI。** 当思考型模型（DeepSeek 等）返回推理内容时，会显示在答案上方的折叠面板 `💭 思考过程` 中。用户可展开查看推理步骤。支持 8 种语言本地化。
- **🔧 Anthropic baseUrl 修复（Issue #141、#134）。** `AnthropicClient` 现在归一化 base URL 中的 `/v1`，防止测试连接时出现 404 错误。自定义 Anthropic 兼容端点正常工作。
- **🔧 gpt-5 max_completion_tokens（Issue #143）。** GPT-5 系列模型现在使用 `max_completion_tokens` 替代 `max_tokens`。截断重试也保留正确的 token key。
- **💬 Query Wiki UX。** 尊重用户配置的 `wikiFolder`（不再硬编码 `wiki/`）。聊天打开时自动滚动到底部。用户消息气泡右对齐。
- **🛡️ 10 项代码审查修复。** 截断重试保留推理内容、所有调用点 `enableThinking` spread 一致性、`activeDocument` null guard、核心字段 `unsupportedFields` 白名单、`wrapReasoningContent` 转义等。
- **🔄 自动迁移。** 从 v1.19.x 升级的旧用户会自动将 `disableThinking` 重置为 `false`——无需手动重新配置。自定义高级设置也重置为默认模式。

我们强烈建议所有用户升级到此版本，以获得更好的 Provider 兼容性和全新的思考 UI。
