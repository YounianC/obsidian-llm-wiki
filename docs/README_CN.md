...
>
> **Obsidian 官方评分 95/100** | 原生支持 9 种语言 | 活跃维护，持续进化
...
[English](../README.md) | **中文文档** | [日本語](README_JA.md) | [한국어](README_KO.md) | [Deutsch](README_DE.md) | [Français](README_FR.md) | [Español](README_ES.md) | [Português](README_PT.md) | [Italiano](README_IT.md)
...
v1.21.0 是一个**次要功能版本**，带来三项重大改进：预摄入需求门控阻止幻觉实体、操作历史面板，以及 Schema 一致性 Phase 1。同时新增意大利语作为第 9 种语言、不完整页面清理器，以及多项 Bug 修复。

- **🛡️ 预摄入需求门控（Issue #164, PR #174）。** 每个源文件在发送给 LLM **之前**都会被验证——空文件/纯空白/仅有 frontmatter 的笔记会被拒绝，阻止小模型/本地模型（如 Ollama）幻觉实体名称。内容哈希去重捕获跨路径的相同文件。交互式摄入在重新摄入重复文件前会提示确认。由 @Indexed-Apogrypha 贡献。
- **📊 操作历史面板（Issue #122, PR #171）。** 在可搜索、可筛选的 UI 中查看最近的摄入、Lint 报告和维护运行。洞察驱动的可视化，包含 KPI 卡片、条目详情和可点击的页面链接。命令面板："View Ingestion History"。
- **🔗 Schema 一致性 Phase 1（Issue #124, PR #167）。** Schema 配置（`schema/config.md`）现在是系统提示词和生成提示词的唯一事实来源。用户定义的自定义 section 和标签词表会注入到每次 LLM 调用中。
- **🇮🇹 意大利语（Issue #159, PR #159）。** 插件 UI 和 Wiki 输出现在支持意大利语作为第 9 种语言。由 @FrancoTampieri 贡献。
- **🧹 不完整页面清理器（Issue #170, PR #177）。** 因中断摄入而留下的部分状态 Wiki 页面会在启动时自动清理。页面会归档到 Obsidian 的 `.trash`（可恢复）。
- **🔧 Bug 修复。** 非中文 UI 中的硬编码中文错误信息（#172）；重复条目膨胀摄入报告计数（#173）。

我们强烈建议所有用户升级到本版本，特别是使用本地 LLM（Ollama、LM Studio）的用户——预摄入门控可阻止"空文件幻觉"这类 Bug（小模型在面对空白 prompt 时会编造实体名称来填充 JSON schema）。
