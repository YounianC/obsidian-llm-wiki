...
**取り込みを高速化するには？**
**設定 → Ingestion Acceleration** で：**Page Generation Concurrency** を3～5に増やし（並列ページ作成）、**Batch Delay** を100～300msに下げます（レート制限に注意）。「Standard」または「Coarse」の**抽出粒度**を選択すると、生成ページ数が減ります。
