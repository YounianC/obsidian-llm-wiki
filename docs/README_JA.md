...
- **⏱️ Batch Delay**: 値が小さいほど高速ですが、rate limitのリスクが高まります。300msから開始し、HTTP 429エラーが発生したら500〜800msに増やしてください。
...
2. **🏷️ Aliasが不足**: aliasがない（v1.7.11より前のページ）場合、LLMは正確なページタイトルでしか一致できません。Lint → Complete Aliasesを実行して修正。
3. **🎯 検索語が一致しない**: ページタイトル、alias、または関連用語をお試しください。LLMはキーワード検索ではなくセマンティックマッチングを行うため — 言い換えが効果的です。

### 🛠️ "Smart Fix All"は何をどの順序で実行しますか？

Smart Fix Allは因果関係の順序で修正を実行し、新たな問題の発生を最小限に抑えます：
1. **Phase 0 — 🏷️ Complete Aliases**: 不足しているaliasを補完し、重複検出が正しく機能するようにします。
...
- **⏱️ Batch Delay** を500ms以上に設定すると呼び出し間隔が広がりますが、token使用量は増えません — 呼び出しの間隔を空けるだけです。
...
5. 🛠️ 修正を選択的に適用 — すべてを一度に修正する必要はありません
