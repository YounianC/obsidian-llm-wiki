...
**v1.11.0 新機能**: 接続テストがコア機能を使用するための必須条件になりました。既存の設定は自動的に移行されます (`llmReady = true`)。プロバイダーやAPIキーを変更した場合は再テストが必要です。

v1.11.0より前に作成されたWiki: **Lint Wiki** を実行して、二重入れ子リンク (`[[[[...]]]]`) とクロスディレクトリスタブ重複を自動修正してください。
...
**5️⃣ 新しい設定を確認（v1.4.0〜v1.7.xで追加）:**
- **🌐 Wiki Output Language**（v1.6.5）: UI言語から独立 — Wikiを日本語で書きながらプラグインUIは英語のまま、またはその逆も可能。
- **📊 抽出粒度**（v1.6.2、v1.10.0拡張）：5つのオプションでLLMがSourceからEntityを抽出する深さを制御：
...
**インストール後、機能が使えないのはなぜですか？(v1.11.0)**
設定 → Karpathy LLM Wiki → プロバイダー選択 → APIキー入力 → Fetch Models → モデル選択 → Test Connection。緑の「LLM Ready」表示が出れば全機能が利用可能になります。

**実行中の取り込み/Lintをキャンセルするには？(v1.11.0)**
ステータスバークリックまたは Ctrl+P → "Cancel current ingestion"。現在のバッチ完了後に安全に停止します。

**log.mdの二重括弧 `[[[[...]]]]` を修正するには？**
Lint Wiki を実行すれば自動検出・修正されます（v1.11.0以降）。
